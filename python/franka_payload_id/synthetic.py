...
def simulate_dynamic_pair(pm: PandaModel, traj: FourierTrajectory, phi_payload: np.ndarray,
    ...
    def meta(loaded: bool) -> RunMetadata:
        return RunMetadata(
            ...
            # Declare the payload truthfully, as the collector now does on hardware.
            m_load=(float(phi_payload[0]) if loaded else 0.0),
            F_x_Cload=([float(v) for v in phi_payload[1:4] / phi_payload[0]]
